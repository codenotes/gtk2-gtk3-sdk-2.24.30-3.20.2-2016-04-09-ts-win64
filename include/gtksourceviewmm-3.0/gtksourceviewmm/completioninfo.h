...
class CompletionInfo : public Gtk::Window
{
  ...
  CompletionInfo& operator=(CompletionInfo&& src) noexcept;

  // noncopyable
  CompletionInfo(const CompletionInfo&) = delete;
  CompletionInfo& operator=(const CompletionInfo&) = delete;
...
public:
  ...
  /** Sets the content widget of the info window. If @a widget does not fit within
   * the size requirements of the window, a Gtk::ScrolledWindow will automatically
   * be created and added to the window.
   * 
   * @deprecated Use Gtk::Container:add() instead. If there is already a child widget, remove it with Gtk::Container::remove().
   * 
